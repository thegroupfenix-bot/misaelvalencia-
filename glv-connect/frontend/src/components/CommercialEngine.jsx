  ...
  return (
    <div style={panelStyle}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(145px, 1fr))", gap: 10 }}>
        ...
        {isLive && summary?.lotWeightGross > 0 && <MiniBox label="Peso Lote Bruto (cargado)" value={fmtNum(summary.lotWeightGross) + " kg"} />}
      ...
      {isLive && specs?.mortalityMargin > 0 && (
        <p style={{ fontSize: 10, opacity: 0.55, margin: "8px 0 0", fontStyle: "italic", borderTop: "1px solid rgba(255,255,255,0.15)", paddingTop: 8 }}>
          Mortalidad en tránsito: {specs.mortalityMargin}% — responsabilidad del comprador. No se deduce del valor comercial ni del peso contractual.
        </p>
      )}
    </div>
  );
