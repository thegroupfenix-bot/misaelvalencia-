(function () {
  ...
  var names = {buy:'Comprar pollo',sell:'Exportar o posicionar producto avícola con GLV',whole:'Pollo entero',parts:'Partes',breast:'Pechuga',leg:'Pierna pernil',thigh:'Muslo',wings:'Alas',feet:'Patas',offal:'Menudencias permitidas',special:'Cortes especiales',frozen:'Congelado',chilled:'Refrigerado, si aplica'};
  window.glvPoultryQuoteDetails = function () {
    if (!['pollo-colombiano','pollo-brasil'].includes(document.getElementById('gn_division').value)) return '';
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!division.querySelector('option[value="pollo-brasil"]')) {
      var brazil = document.createElement('option');
      brazil.value = 'pollo-brasil';
      brazil.textContent = 'Pollo brasileño';
      division.appendChild(brazil);
    }
    var params = new URLSearchParams(location.search);
    if (['pollo-colombiano','pollo-brasil'].includes(params.get('division'))) {
      division.value = params.get('division');
      var role = params.get('poultry');
      if (role === 'buy' || role === 'sell') document.getElementById('poultry_role').value = role;
    }
    function sync() {
      var active = ['pollo-colombiano','pollo-brasil'].includes(division.value);
      group.hidden = !active;
